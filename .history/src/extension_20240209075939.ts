  ...
  let disposableKeyListener = vscode.workspace.onDidChangeTextDocument(event => {
    ...
    event.contentChanges.forEach(change => {
      ...
      if (isInProgress === true && startLine !== null) {
        lineRange.push({
          startLine: startLine + 1, // Further increase the start line by 1 when adding to lineRange
          endLine: currentLine + 1, // Increase the end line by 1 from the current line
          text: change.text,
        });
        isInProgress = false;
      }
