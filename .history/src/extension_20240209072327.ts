...
export function activate(context: vscode.ExtensionContext) {
  let disposable = vscode.commands.registerCommand(
    ...
    () => {
      vscode.window.showInformationMessage("Copilot dectector started!");
    }
  );
  ...
  let disposableKeyListener = vscode.workspace.onDidChangeTextDocument(
    (event) => {
      ...
      event.contentChanges.forEach((change) => {
        ...
        // Improved logic to detect likely Copilot suggestions
        if (textLength > 2 && likelyCopilotSuggestion(change)) {
  ...
  function likelyCopilotSuggestion(change: vscode.TextDocumentContentChangeEvent): boolean {
    // Implement logic here to determine if a change is likely from Copilot
    // This could involve checking the speed of changes, the type of change, etc.
