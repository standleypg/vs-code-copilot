  ...
  let disposableKeyListener = vscode.workspace.onDidChangeTextDocument(event => {
    ...
    event.contentChanges.forEach(change => {
      ...
      if (change.text.length > 2 && likelyCopilotSuggestion(change)) {
        startLine = currentLine + 1; // Corrected: This was intended to increase start line by 1 but consider the context of usage
        isInProgress = true;
      }

      if (isInProgress === true && startLine !== null) {
        lineRange.push({
          startLine, // Note: Ensure the logic of startLine increment aligns with your intention
          endLine: currentLine, // Now set to the line where the cursor is currently active
          text: change.text,
        });
