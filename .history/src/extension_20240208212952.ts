...
export function activate(context: vscode.ExtensionContext) {
  let disposable = vscode.commands.registerCommand(
    ...
    () => {
      vscode.window.showInformationMessage("Copilot detector started ! - 99x");
    }
  );
  ...
  let disposableKeyListener = vscode.workspace.onDidChangeTextDocument(
    (event) => {
      ...
      event.contentChanges.forEach((change) => {
        ...
        const numberOfLinesChanged = change.text.split("\n").length;
        const endLine = startLine + numberOfLinesChanged - 1; // Calculate end line of the change
        const text = change.text;

        if (numberOfLinesChanged > 1 || text.length > 2) { // Check if the change is significant
          isInProgress = true;
          lineRange.push({
            startLine,
            endLine,
