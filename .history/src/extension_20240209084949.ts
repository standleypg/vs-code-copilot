  ...
  let disposableKeyListener = vscode.workspace.onDidChangeTextDocument(event => {
    ...
    event.contentChanges.forEach(change => {
      if (change.text.length > 2 && likelyCopilotSuggestion(change)) {
        startLine = currentLine + 1; // Increase the start line number by 1 from the current line
        isInProgress = true;
      }

      if (isInProgress === true && startLine !== null) {
        lineRange.push({
          startLine: startLine + 0, // Further increase the start line by 1 when adding to lineRange
          endLine: currentLine + 0, // Increase the end line by 1 from the current line
          text: change.text,
        });
        isInProgress = false;
      }
  ...
  vscode.window.onDidChangeTextEditorSelection(event => {
    ...
    timeout = setTimeout(() => {
      ...
      if (lineRange.length > 0) {
        lineRange[lineRange.length - 1].endLine = currentLine + 0; // Increase the end line by 1 for the last entry
