...
let isAcceptingCompletion = false; // Flag to track specific keypresses

export function activate(context: vscode.ExtensionContext) {
  context.subscriptions.push(vscode.commands.registerCommand('yourExtension.detectCtrlI', () => {
...
let disposableKeyListener = vscode.workspace.onDidChangeTextDocument(event => {
  if (!isAcceptingCompletion) return; // Only proceed if the flag is true
